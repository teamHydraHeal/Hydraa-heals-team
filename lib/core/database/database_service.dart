...
class DatabaseService {
    ...
    await db.transaction((txn) async {
      ...
      await txn.execute('''
        CREATE TABLE $riskAnalysisTable (
          ...
          is_synced INTEGER NOT NULL DEFAULT 0,
          created_at TEXT NOT NULL,
          FOREIGN KEY (district_id) REFERENCES $districtsTable (id)
        )
      ''');
      ...
      await txn.execute('''
        CREATE TABLE $mcpCardsTable (
          ...
          created_at TEXT NOT NULL,
          updated_at TEXT NOT NULL,
          FOREIGN KEY (asha_worker_id) REFERENCES $usersTable (id)
        )
      ''');
  ...
  static Future<void> _createIndexes(Database db) async {
    ...
    await db.execute('CREATE INDEX idx_health_reports_user_id ON $healthReportsTable (user_id)');
    await db.execute('CREATE INDEX idx_health_reports_district_id ON $healthReportsTable (district_id)');
    await db.execute('CREATE INDEX idx_health_reports_severity ON $healthReportsTable (severity)');
    await db.execute('CREATE INDEX idx_health_reports_reported_at ON $healthReportsTable (reported_at)');
    await db.execute('CREATE INDEX idx_health_reports_status ON $healthReportsTable (status)');
    await db.execute('CREATE INDEX idx_health_reports_location ON $healthReportsTable (latitude, longitude)');
  ...
  static Future<void> _upgradeDatabase(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2) {
      ...
      await db.execute('ALTER TABLE $usersTable ADD COLUMN last_login TEXT');
      await db.execute('ALTER TABLE $usersTable ADD COLUMN is_active INTEGER NOT NULL DEFAULT 1');
      await db.execute('ALTER TABLE $usersTable ADD COLUMN profile_data TEXT');
      await db.execute('ALTER TABLE $usersTable ADD COLUMN verification_documents TEXT');
      
      await db.execute('ALTER TABLE $healthReportsTable ADD COLUMN block_id TEXT');
      await db.execute('ALTER TABLE $healthReportsTable ADD COLUMN village_id TEXT');
      await db.execute('ALTER TABLE $healthReportsTable ADD COLUMN sync_attempts INTEGER NOT NULL DEFAULT 0');
      await db.execute('ALTER TABLE $healthReportsTable ADD COLUMN last_sync_attempt TEXT');
      
      await db.execute('ALTER TABLE $districtsTable ADD COLUMN iot_sensor_count INTEGER NOT NULL DEFAULT 0');
      await db.execute('ALTER TABLE $districtsTable ADD COLUMN health_centers_count INTEGER NOT NULL DEFAULT 0');
      await db.execute('ALTER TABLE $districtsTable ADD COLUMN asha_workers_count INTEGER NOT NULL DEFAULT 0');
    }
  }
